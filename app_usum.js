(() => {
  ...
  const searchInput = document.getElementById('search-usum');
  searchInput.parentNode.appendChild(suggestionBox);
  const resultDiv = document.getElementById('result-usum');
  ...
  searchInput.addEventListener('input', function () {
    ...
    if (searchText.length >= 2) {
      for (let pokemon of pokemons) {
        // 울트라썬문은 7세대이므로 전국도감 807번(제라오라)까지만 허용
        if (pokemon.name.includes(searchText) && parseInt(pokemon.number) <= 807) {
  ...
  searchInput.addEventListener('keyup', function (event) {
    if (event.key === 'Enter') {
      const searchText = searchInput.value;
      const matchedPokemon = pokemons.find(p => p.name.toLowerCase() === searchText.toLowerCase() && parseInt(p.number) <= 807);

      if (matchedPokemon) {
        displayPokemonData(matchedPokemon);
      } else {
        alert("USUM 도감에 존재하지 않거나 잘못된 이름입니다. (807번 제라오라까지만 검색 가능합니다)");
      }
  ...
  // 버튼 클릭 이벤트 처리
  const searchButton = document.getElementById('search-button-usum');
  ...
  window.searchPokemonUSUM = function (pokemonName) {
    if (!pokemons.length) {
      setTimeout(() => window.searchPokemonUSUM(pokemonName), 100);
      return;
    }
    const matchedPokemon = pokemons.find(p => p.name.toLowerCase() === pokemonName.toLowerCase() && parseInt(p.number) <= 807);
    if (matchedPokemon) {
  ...
  window.navigateToMoveSearchUSUM = function (moveName) {
    window.location.hash = '#move';
    if (typeof window.searchMoveByName === 'function') {
      window.searchMoveByName(moveName, 'usum');
    }
  };
  ...
  function formatSkillListToBadges(skillsArray) {
    ...
    skillsArray.forEach(skillStr => {
      ...
      if (match) {
        ...
        const skillName = match[2].replace(/'/g, "\\'"); // 예: 할퀴기
        // 나중에 클릭 이벤트를 달 수 있도록 a 태그로 렌더링
        badgesHtml += `<a href="javascript:void(0);" onclick="window.navigateToMoveSearchUSUM('${skillName}')" class="skill-badge"><span class="badge-label">${prefix}</span>${skillName}</a>`;
      } else {
        // 괄호 패턴이 없는 경우 (예: 교배기)
        const safeSkill = cleanSkillStr.replace(/'/g, "\\'");
        badgesHtml += `<a href="javascript:void(0);" onclick="window.navigateToMoveSearchUSUM('${safeSkill}')" class="skill-badge">${cleanSkillStr}</a>`;
      }
    });
    badgesHtml += '</div>';
    return badgesHtml;
  ...
  async function displayPokemonData(pokemon) {
    ...
    let tutorSkills = formatSkillListToBadges(pokemon.RelearnSkills);
    ...
    <div class="card mb-3 fade-in">
      ...
      return `<div class="stat-bar" style="width: ${widthPercent}%;">${translatedStat}: ${statValue}</div>`;
        ...
        <div class="move-tabs">
          <ul class="nav nav-tabs" id="moveTabsUsum-${pokemon.number}" role="tablist">
            <li class="nav-item" role="presentation">
              <a class="nav-link active" id="levelup-usum-tab-${pokemon.number}" data-toggle="tab" href="#levelup-usum-${pokemon.number}" role="tab" aria-controls="levelup-usum-${pokemon.number}" aria-selected="true">자력기</a>
            </li>
            <li class="nav-item" role="presentation">
              <a class="nav-link" id="machine-usum-tab-${pokemon.number}" data-toggle="tab" href="#machine-usum-${pokemon.number}" role="tab" aria-controls="machine-usum-${pokemon.number}" aria-selected="false">기술머신</a>
            </li>
            <li class="nav-item" role="presentation">
              <a class="nav-link" id="egg-usum-tab-${pokemon.number}" data-toggle="tab" href="#egg-usum-${pokemon.number}" role="tab" aria-controls="egg-usum-${pokemon.number}" aria-selected="false">교배기</a>
            </li>
            <li class="nav-item" role="presentation">
              <a class="nav-link" id="tutor-usum-tab-${pokemon.number}" data-toggle="tab" href="#tutor-usum-${pokemon.number}" role="tab" aria-controls="tutor-usum-${pokemon.number}" aria-selected="false">가르침 기술</a>
            </li>
          </ul>
          <div class="tab-content" id="moveTabsUsumContent-${pokemon.number}">
            <div class="tab-pane fade show active" id="levelup-usum-${pokemon.number}" role="tabpanel" aria-labelledby="levelup-usum-tab-${pokemon.number}">
              ${levelUpSkills}
            </div>
            <div class="tab-pane fade" id="machine-usum-${pokemon.number}" role="tabpanel" aria-labelledby="machine-usum-tab-${pokemon.number}">
              ${machineSkills}
            </div>
            <div class="tab-pane fade" id="egg-usum-${pokemon.number}" role="tabpanel" aria-labelledby="egg-usum-tab-${pokemon.number}">
              ${eggSkills}
            </div>
            <div class="tab-pane fade" id="tutor-usum-${pokemon.number}" role="tabpanel" aria-labelledby="tutor-usum-tab-${pokemon.number}">
              ${tutorSkills}
            </div>
          </div>
        </div>
